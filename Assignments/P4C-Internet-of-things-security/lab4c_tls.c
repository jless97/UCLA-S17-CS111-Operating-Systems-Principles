...
SSL *ssl_init(void) {
    ...
    if (sslClient == NULL) {
        fprintf(stderr, "Error: can't get SSL structure.\n");
        exit(EXIT_OTHER_FAILURE);
    }
...
button_handler(void) {
    ...
    // Printing twice for some reason, add flag
    if (send_flag == 0)
        SSL_write(sslClient, report_entry, ENTRY_BUFFER_SIZE);
    send_flag = 1;
    if (log_flag == 1) {
...
poll_service_server(void) {
    ...
    int i;
    nread = SSL_read(sslClient, buf, BUFFER_SIZE);
    if (nread < 0) {
...
main (int argc, char *argv[])
{
    ...
    // Send to server
    ssize_t send_status;
    //send_status = SSL_write(sslClient, initial_send_buf, 14);
    snprintf(initial_send_buf, 14, "ID=%s\n", id_buf);
    SSL_write(sslClient, initial_send_buf, 14);
    if (send_status < 0) {
