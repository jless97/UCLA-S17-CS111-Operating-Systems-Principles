...
main (int argc, char *argv[])
{
    ...
    // Total run time (ns)
    long total_runtime = end_time.tv_nsec - start_time.tv_nsec;
    // Avg. time per operation (ns)
    long avg_runtime = total_runtime / noperations;
    // Final count
    ...
    // Final count (should be 0) (if not 0, then sychronization issues/race conditions)
    if (final_count != 0) {
        fprintf(stderr, "Error: Final count not correct due to synchronization issues.\n");
        exit(EXIT_FAILURE);
    }
    
    // If count is 0, success
    exit(EXIT_SUCCESS);
}
